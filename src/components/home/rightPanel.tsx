...
export const RightPanel = () => {
    ...
    const totalBalance = pedidos.reduce((acc, pedido) => acc + pedido.total, 0);
    ...
    const handleResize = () => {
        if (window.innerWidth >= 768) {
            setIsExpanded(true); // Siempre expandido en pantallas grandes
        }
    };
    ...
    return (
        <aside className={`transition-all duration-300 bg-white shadow-md p-6 flex flex-col justify-between text-sm 
        ${isExpanded ? "md:w-96 w-full md:h-screen" : "w-full h-auto"}`}>

            {/* Barra Superior: Solo para móvil */}
            <div className="flex justify-between items-center md:hidden mb-4">
                <h4 className="text-gray-700 font-semibold text-sm">Resumen Pedido</h4>
                <button
                        ...
                        {pedido.items.map((item) => (
                            <div key={item.item_id} className="flex justify-between items-center">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs">{item.nombre} x {item.cantidad}</span>
                                </div>
